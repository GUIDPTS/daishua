...
function getPoint() {
	...
	if(inputsname!=''){
		$.each(inputsname.split('|'), function(i, value) {
			...
			if(value=='说说ID'||value=='说说ＩＤ')
				...
				var addstr='<div class="layui-btn layui-btn-warm onclick" onclick="get_kuaishou(\'inputvalue'+(i+2)+'\',$(\'#inputvalue\').val())">自动获取作品ＩＤ</div>';
			else if(value=='抖音评论ID')
				var addstr='<div class="layui-btn layui-btn-warm onclick" onclick="getCommentList(\'inputvalue'+(i+2)+'\',$(\'#inputvalue\').val())">自动获取</div>';
			else if(value=='收货地址'||value=='收货人地址')
				var addstr='<div class="layui-btn layui-btn-warm onclick" onclick="getCity(\'inputvalue'+(i+2)+'\')">点此选择</div>';
			else
				var addstr='';
	...
	if($("#inputname").html() == '快手ID'||$("#inputname").html() == '快手ＩＤ'||$("#inputname").html() == '快手用户ID'){
		...
		if($("#inputname2").html() == '作品ID'||$("#inputname2").html() == '作品ＩＤ'||$("#inputname2").html() == '快手作品ID'){
			$('#inputvalue2').attr("placeholder", "填写作品链接后点击获取");
