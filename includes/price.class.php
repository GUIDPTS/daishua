...
class Price {
	...
	public function setiPriceInfo($tid,$price){
		...
		return $DB->query("update shua_site set iprice='$iprice_data' where zid='{$this->zid}'");
	}
	...
	private function getSiteInfo($zid){
		global $DB;
		$data = $DB->get_row("SELECT zid,upzid,power,price,iprice,endtime FROM shua_site WHERE zid='$zid' limit 1");
		return $data;
	}
