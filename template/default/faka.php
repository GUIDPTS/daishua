...
if($rcount<$count){
	$scount = $count-$rcount;
	$rs=$DB->query("SELECT * FROM shua_faka WHERE tid='{$row['tid']}' AND orderid=0 ORDER BY kid ASC LIMIT {$scount}");
	while($res = $DB->fetch($rs))
	{
		if(!empty($res['pw'])){
			$kmdata.='卡号：'.$res['km'].' 密码：'.$res['pw']."\r\n";
		...
		$DB->query("update `shua_faka` set `orderid`='$orderid',`usetime`='$date' where `kid`='{$res['kid']}'");
	}
}
?>
<!DOCTYPE html>
