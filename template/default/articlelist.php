...
$msgcount=$DB->count("SELECT count(*) FROM shua_message WHERE type IN ($type){$sql} AND active=1");
$rs=$DB->query("SELECT * FROM shua_message WHERE type IN ($type){$sql} AND active=1 ORDER BY id DESC LIMIT 0,$limit");
$msgrow=array();
