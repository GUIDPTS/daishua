...
case 'cleariprice':
	$zid=intval($_POST['zid']);
	if($DB->query("update shua_site set iprice=NULL where zid='{$zid}'"))
		exit('{"code":0,"msg":"succ"}');
	else
...
case 'transfer':
	...
	$res = $DB->get_row("SELECT * FROM shua_tixian WHERE id='$id' AND status=0");
	...
	if (isset($json['code']) && $json['code']) {
		if(!$DB->query("update shua_tixian set status=1,endtime=NOW() where id='$id'")) exit(json_encode(array('code'=>0,'msg'=>'汇款成功!但是结算记录状态改变失败！')));
